...
#if !defined(DUK_F_WINDOWS) && defined(DUK_USE_DEBUGGER_SUPPORT)
...
DUK_INTERNAL void duk_trans_socket_init(void) {
...
DUK_INTERNAL void duk_trans_socket_finish(void) {
	if (client_sock >= 0) {
...
DUK_INTERNAL void duk_trans_socket_waitconn(void) {
	struct sockaddr_in addr;
	socklen_t sz;
...
DUK_INTERNAL duk_size_t duk_trans_socket_read_cb(void *udata, char *buffer, duk_size_t length) {
	ssize_t ret;
...
DUK_INTERNAL duk_size_t duk_trans_socket_write_cb(void *udata, const char *buffer, duk_size_t length) {
	ssize_t ret;
...
DUK_INTERNAL duk_size_t duk_trans_socket_peek_cb(void *udata) {
#if defined(USE_SELECT)
	struct timeval tm;
	fd_set rfds;
	int select_rc;
...
DUK_INTERNAL void duk_trans_socket_read_flush_cb(void *udata) {
	(void) udata;  /* not needed by the example */
...
DUK_INTERNAL void duk_trans_socket_write_flush_cb(void *udata) {
	(void) udata;  /* not needed by the example */
